...
>(
  ({
    ...
    const handleDeletePendingAction = async (actionId: number) => {
      if (
        window.confirm(
          "Are you sure you want to delete this offline lead? This action cannot be undone."
        )
      ) {
        try {
    ...
    useEffect(() => {
      // let sourceDataArray =
      //   propLeadData && propLeadData.length > 0
      //     ? propLeadData
      //     : isOnline
      //     ? data?.leadData?.aaData
      //     : cachedData?.leadData?.aaData;

      let sourceDataArray = isOnline
        ? data?.leadData?.aaData
        : propLeadData && propLeadData.length > 0
        ? propLeadData
        : cachedData?.leadData?.aaData;
    ...
    const checkNavigateEscalatedLead = async (caseId: string) => {
      try {
        ...
        if (InitiatedToId === currentUserId) {
          dispatch(
            updateLeadId({
              leadId: res?.responseData?.leadHeaderDetail?.leadId,
            })
          );
          dispatch(updateContactData(res.responseData));
          dispatch(
            updateLeadHeaderDetails(res?.responseData?.leadHeaderDetail)
          );
          navigate(`/contact/${caseId}?case=Contact`);
        } else {
          toastFail(
            "You are not the intended recipient of this escalated lead"
          );
        }
      } catch (err) {
    ...
    const columns: ColumnsType<IWorklistTableColumnType> = [
      ...
      // },
      {
        ...
        render: (value: string | number) =>
          value
            ? formatINR(Number(String(value).replace(/[^0-9.-]/g, "")))
            : "",
      },
      {
                      ...
                      {single._isOffline && (
                        <Box mb={3}>
                          <HStack spacing={2} mb={2}>
                            ...
                            {isOnline &&
                              single._pendingActionId &&
                              !syncingItems.has(single._pendingActionId) &&
                              single._syncStatus !== "error" && (
                                <Button
                                  size="xs"
                                  colorScheme="blue"
                                  onClick={e => {
                                    e.stopPropagation();
                                    handleSyncSingleAction(
                                      single._pendingActionId
                                    );
                                  }}
                                  isLoading={syncingItems.has(
                                    single._pendingActionId
                                  )}
                                >
                                  Sync Now
  ...
  /> */
                              <Checkbox
                                size="lg"
                                colorScheme="blue"
                                isChecked={allID.includes(
                                  String(single.leadId)
                                )}
                                onChange={() => handleSelect(single)}
                                onClick={e => e.stopPropagation()}
                              />
